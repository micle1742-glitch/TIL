...
console.log(notificationCount??10)
console.log('상태메세지:',statusMessage??'상태 메세지 없음');
...
// 문제 3 풀이
function product(name, price1, stock) {
    this.상품명 = name;
    this.가격원 = price1;
    this.재고 = stock;

    this.getInfo = function () {
        return `${this.상품명}: ${this.가격원} ${this.재고}`;
    };
}
const 키보드 = new product('키보드', 50000, 3);
const 마우스 = new product('마우스', 30000, 5);
const 키보드1 = new product('키보드', 50000, 2);
const 마우스1 = new product('마우스', 30000, 5);
console.log(키보드.getInfo());
console.log(마우스.getInfo());
console.log(키보드1.getInfo());
console.log(마우스1.getInfo());
...
console.log(`${student1.이름}: ${student1.getGrade()}`);
console.log(`${student2.이름}: ${student2.getGrade()}`);
...
// 문제 5 풀이
function 멤버(name,option){
    this.name = name;
    this.option = option;
}
멤버.prototype.introduce = function () {
    const 별명 = this.option?.nickname??this.name
    const 등급 = this.option?.level??'일반'
    return `${별명}님은 ${등급}회원입니다.`
}

const 멤버1 = new 멤버('홍길동',{
    nickname: '길동이',
    level: 'vip'
})

const 멤버2 = new 멤버('유관순')

console.log(멤버1.introduce())
console.log(멤버2.introduce())
console.log(멤버1.introduce===멤버2.introduce)
