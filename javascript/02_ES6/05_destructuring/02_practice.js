...
const orders = [
 ...
 const 상품정보 = orders.map(({product, price, quantity}) => `${product} 결제금액원${price*quantity}`)
 const oderLabels = 상품정보
 console.log(oderLabels)
...
for (const 히스토리나누기 of searchHistory) {
}
searchHistory.forEach ((히스토리나누기,index) => 
    console.log(index+1,히스토리나누기))
...
console.log('원본 앞팀:',frontTeam)
console.log('원본 뒤팀:',backTeam)
...
const {id,name,role,skills} = user
const userName = name;
console.log('사용자명',userName);
console.log('역할:',role)
user.nickname = '이름없음'
console.log('별명:',user.nickname)
function printTech (리더, ...멤버) {
    console.log('주요기술:',리더);
    console.log('나머지기술:',멤버);
    }
printTech(...skills)
...
//proucts와 newproduct합치기
const addedProducts = [...products , newProduct]

//뒤부터읽으면 편함 product변수만듬 만약 id가1이면 product배열이price가져와서 55000원으로 바꿈 그리고 나머지 product반환
//그리고 updatedProducts로 저장
...
//필터이용해서 product의 stock>0보다큰값을 걸러내 저장한거임
const availableProducts = updatedProducts.filter((product) => {
    
    return product.stock > 0;
})

//저장한거를 이름만꺼내서 저장 참고로 map이면배열로저장임
const result = availableProducts.map(({name}) => {
    return name});

//업데이트상품에서 가격만뽑기 위랑같음
const 가격뽑기 = updatedProducts.map(({price}) => {
    return price});

//  ...prices변수 (안에값없음)만들고 price가져와서 합계라는변수만들어서 총액값구하는 함수임
function getTotal(...prices) {
    return prices.reduce((합계,price) => 합계 +price,0)
}

console.log('원본 상품',products[0].stock)
console.log('추가 후 상품 수',addedProducts[3].stock)
console.log('원본 키보드 가격:',products[0].price)
console.log('변경된 키보드 가격:',updatedProducts[0].price)
console.log('판매가능상품',result)
console.log(getTotal(...가격뽑기))